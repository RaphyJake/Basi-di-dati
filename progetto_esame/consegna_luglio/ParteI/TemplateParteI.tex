...
\item \textbf{SQUADRE}(\ul{codSquadra}, nome)
\item \textbf{BRANI}(\ul{titolo}, \underline{codArtista}, genereMusicale, durata)
\item \textbf{SERATE}(\ul{nome}, data)
\item \textbf{VOTI}(\ul{codVoto},tipo, dataOra)
\item \textbf{FORMAZIONI}(\underline(codArtista), \underline{codSquadra}, \underline(nomeSerata), dataModifica)
